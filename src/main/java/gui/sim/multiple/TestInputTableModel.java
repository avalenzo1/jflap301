...
public class TestInputTableModel extends InputTableModel {
	/**
	 *
	 */
	 ...
	 */
	public TestInputTableModel(Automaton automaton, int add) {
		super(2 * inputsForMachine(automaton)+ 1 + add);
		System.out.println(columns);

	}

	public TestInputTableModel(Grammar gram, int add) {
		super(2 * 1 + 1 + add);
	}
	 ...
	 *            the model to copy
	 */
	public TestInputTableModel(TestInputTableModel model) {
		super(model);
	}
	 ...
	 */
	protected TestInputTableModel(int columns) {
		super(columns);
	}
	 ...
	 */
	public static TestInputTableModel getModel(Automaton automaton, boolean multipleFile) {
		TestInputTableModel model = (TestInputTableModel) INPUTS_TO_MODELS
				.get(new Integer(inputsForMachine(automaton)));
		if (model != null && (model.isMultiple == multipleFile)) {
			model = new TestInputTableModel(model);
			...
			model = new TestInputTableModel(automaton, add);
		}
	...
	public static TestInputTableModel getModel(Grammar gram, boolean multipleFile) {
		TestInputTableModel model = (TestInputTableModel) INPUTS_TO_MODELS
				.get(new Integer(1));
		if (model != null) {
			model = new TestInputTableModel(model);
            ...
            int add = 0;
            if(multipleFile) add = 1;
			model = new TestInputTableModel(gram, add);
		}
		model.addTableModelListener(LISTENER);
        if(multipleFile){
            model.isMultiple = true;
	...
	public void setResult(int row, String result, Configuration config, ArrayList<String> comparison, int index) {
        ...
        if(config instanceof TMConfiguration && config != null){
			...
			// TODO: MAke sure theres no bugs here
...
//				setValueAt("", row, halfway + i);
	...
	protected final static TableModelListener LISTENER = new TableModelListener() {
		public void tableChanged(TableModelEvent event) {
			TestInputTableModel model = (TestInputTableModel) event.getSource();
	 ...
	 */
	protected final static Map<Integer, TestInputTableModel> INPUTS_TO_MODELS = new HashMap<>();
